...
// Native syntax highlighter for Vanta — applies colour attributes to an
// NSTextStorage by scanning the text (comments, strings, numbers, keywords,
// builtins, operators).
enum VantaSyntax {
    ...
    static let builtins: Set<String> = [
        ...
        "floor","ceil","sqrt","power","sin","cos","tan","log","exp","sum","product","min",
        "max","random","now","today","clock","first","last","range","contains","keys","values",
        "sort","reverse","slice","push","pop","remove_at","map","keep","reduce","count_where",
        "find_where","sort_by","type_of","is_a","is_number","is_text","is_list","is_map",
        "is_function","is_nothing","matches","find_all","replace_all","fail","assert","read_file",
        "write_file","read_bytes","band","bor","bxor","bnot","shift_left","shift_right","run",
        "shell","arguments","env","to_json","from_json","pi","e",
    ]
    ...
    static func highlight(_ storage: NSTextStorage, font: NSFont) {
        ...
        storage.setAttributes([.font: font, .foregroundColor: Theme.nink], range: full)

        func isDigit(_ c: unichar) -> Bool { c >= 48 && c <= 57 }
        func isAlpha(_ c: unichar) -> Bool { (c >= 65 && c <= 90) || (c >= 97 && c <= 122) || c == 95 }
        func color(_ col: NSColor, _ loc: Int, _ len: Int) {
        ...
        while i < n {
            ...
            if isAlpha(c) {
                ...
                if keywords.contains(word) { color(Theme.nkw, i, j - i) }
                else if builtins.contains(word) { color(Theme.nfn, i, j - i) }
                i = j
                continue
            }
            if operators.contains(c) { color(Theme.nop, i, 1) }
            i += 1
        }
        storage.endEditing()
    }
